...
def read_log_line(line: str):
    ...
    if log_message == "ingestion-start":
        ...
        if ingested_files:
            ingestion_data = {"id": ingestion_id, "ingestion_type": ingestion_type, "duration": ingestion_duration.total_seconds(), "num_of_files": ingested_files}
...
with open("ingestions.csv", "a") as f:
    lines = []
    lines.append("id,ingestion_type,duration,num_of_files\n")
    for ingestion_data in ingestions:
        data_line = f"{ingestion_data['id']},{ingestion_data['ingestion_type']},{ingestion_data['duration']},{ingestion_data['num_of_files']}\n"
